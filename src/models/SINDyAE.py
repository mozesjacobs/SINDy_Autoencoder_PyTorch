...
class Net(nn.Module):
    ...
    def forward(self, x, dx, lambdas):
        ...
        theta = sindy_library(z, self.poly_order, self.use_sine, self.include_constant) # (b * T) x 20
    ...
    def get_derivative(self, layer_output, dL, net):
        ...
        for i in range(len(net) - 1):
            ...
            if isinstance(curr_layer, nn.Linear):
                ...
                wT = w.T
                layer_output = torch.nn.functional.sigmoid(torch.matmul(layer_output, wT) + b)
